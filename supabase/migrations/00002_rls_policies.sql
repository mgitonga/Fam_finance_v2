...
CREATE POLICY "Admins can update own household"
  ...
  USING (id = public.get_household_id() AND public.get_user_role() = 'admin');
...
CREATE POLICY "Admins can update household members"
  ON users FOR UPDATE
  USING (household_id = public.get_household_id() AND public.get_user_role() = 'admin');
...
CREATE POLICY "Admins can create accounts"
  ...
  WITH CHECK (household_id = public.get_household_id() AND public.get_user_role() = 'admin');
...
CREATE POLICY "Users can create transactions"
  ...
  WITH CHECK (household_id = public.get_household_id() AND user_id = auth.uid());
...
CREATE POLICY "Users can view goal contributions"
  ...
  USING (
    goal_id IN (
      SELECT id FROM savings_goals WHERE household_id = public.get_household_id()
    )
  );
